...
void draw_line(mlx_image_t *img, float **converted_matrix, int start, int end)
{
	...
	if (line->dx == 0)
		draw_vertical_line(line, converted_matrix, start, end);
	else if (line->dy == 0)
		draw_horizontal_line(line, converted_matrix, start, end);
	else
	{
		if (line->dx >= line->dy)
			draw_line_larger_x_axis(line, converted_matrix, start, end);
		else
			draw_line_larger_y_axis(line, converted_matrix, start, end);
	}
	free(line);
}