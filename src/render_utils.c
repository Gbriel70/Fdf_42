...
t_matrix_dimensions *get_matrix_dimensions(float **map_matrix, t_map *s_map)
{
	...
	s_matrix_dimensions = (t_matrix_dimensions *)malloc(sizeof(t_matrix_dimensions));
...
float **scale_dimension_matrix(t_map *s_map, float **map_matrix, t_matrix_dimensions *s_matrix_dimensions)
{
	...
	while (s_map)
	{
		...
		converted_matrix[x][y] = (map_matrix[x][y] * scale) + (SCREEN_WIDTH / 2) - (s_matrix_dimensions->matrix_width / 2 * scale);
		converted_matrix[x][y + 1] = (map_matrix[x][y + 1] * scale) + (SCREEN_HEIGHT / 2) - (s_matrix_dimensions->matrix_height / 2 * scale);
		x++;
		s_map = s_map->next;
	}
